...
#window.state('zoomed')
window.iconbitmap(relative_to_assets("LiClaw.ico"))
...
def make_db():
    ...
    from assets.db_generator import make_a_DB
...
def checked_clear_begin_search(predefined_db_path,output_parent_directory,raw_file_formatted_path,target_path):
    from assets.database_search import launch_db_search_pt1
    from assets.PSM_assignment import PSM_assignment_execute
    from assets.motif_search import start_motif_search
    from assets.results_metric_extract import results_metric_extract_start
    from assets.metric_handling import metric_handling_apply
    from assets.target_decoy_assess import target_decoy_apply
    ...
    if len(database_csv_path.get()) > 0:
        ...
        if len(target_peptide_list_path.get()) > 0:
            if len(input_path_format_MS2.get()) > 0:
                ...
                if len(input_path_MS2.get()) > 0:
                    from assets.format_MS2_file import format_raw_MS2
